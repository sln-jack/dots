...
def dotnet(d: Path, v: str, extra_vs: list = []):
    ...
    sh(f'mkdir {d}/bin && ln -sf ../dotnet {d}/bin/')
...
def roslyn_ls(d: Path, v: str):
    ...
    sh(f'mkdir {d}/bin && ln -sf ../Microsoft.CodeAnalysis.LanguageServer {d}/bin/')
...
if role('selini') and sys == 'linux':
    ...
    # Networking
    #wireshark('4.6.2')
    wireshark('4.2.0-sln') # selini fork
    termshark('2.4.0')
